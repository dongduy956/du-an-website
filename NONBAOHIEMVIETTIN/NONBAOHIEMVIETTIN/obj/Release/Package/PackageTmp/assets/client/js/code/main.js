...
$(function () {
  ...
  $("#passoldchange").keypress(function (e) {
    if (e.which == 13) $("#btnsubmitchangepassword").click();
  });
  ...
  $(".nice-select li")
    ...
    .click(function () {
      ...
      if (val == "1") {
        $("#list-item .col-lg-4.col-md-4.col-sm-6")
          .sort(desc_sortName)
          .appendTo("#list-item");
        $("#list .product_list_item.mb-35")
          .sort(desc_sortName_1)
          .appendTo("#list");
      } else if (val == "0") {
        $("#list-item .col-lg-4.col-md-4.col-sm-6")
          .sort(asc_sortName)
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.status == "1") {
          ...
          swal("Thông báo", "Mời bạn đăng nhập!", "warning").then((value) => {
            location.href = "/dang-nhap";
          });
  ...
  //Xoá 1 yêu thích
  $("#lst-wish,#table-wish").click(function (e) {
    if (e.target.closest(".deletewish")) {
      e.preventDefault();
      var ProductId = $(e.target.parentElement).data("id")||$(e.target).data('id');
      swal({
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.status == "1") {
          ...
          setTimeout(() => $("#wish-" + ProductId).remove(), 500);
          $("#lst-wish .block_content p").text(`${data.sumQuantity} sản phẩm.`);
          if (check)
            iziToast.success({
              timeout: 1500,
          ...
          iziToast.error({
            ...
            title: "Lỗi",
            message:
              "Lỗi hệ thống khi xoá sản phẩm. Vui lòng thao tác lại sau.",
            position: "topRight",
          });
      },
      error: function (data) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.status == "1") {
          ...
          $(".cart-" + data.id + " .quantity").text(
            "Số lượng:" + data.quantity
          );
          $("#lst-cart .prices").text(data.sumMoney);
          if (location.pathname == "/yeu-thich") deletewish(ProductId, false);
          $(".shopping_cart a span").text(
            `${data.sumQuantity} sản phẩm-${data.sumMoney}`
          );
        } else if (data.status == "0") {
  ...
  $(".btnaddcart")
    ...
    .click(function () {
      ...
      if (typeof Quantity == "undefined") {
        ...
        else if (classes == "form form-category")
          Quantity = $("#txtquantity-category-" + ProductId).val();
        else if (classes == "form form-group-product")
          Quantity = $("#txtquantity-group-" + ProductId).val();
        else Quantity = $("#txtquantity-detail").val();
      }

      addCart(ProductId, Quantity);
    });
  //Event xoá giỏ hàng
  $("#cart,#table-cart").click(function (e) {
    if (e.target.closest(".deletecart")) {
      e.preventDefault();
      e.stopPropagation();
      var ProductId = $(e.target.parentElement).data("id");
      swal({
        title: "Bạn chắc chắn xoá sản phẩm này?",
        icon: "warning",
        buttons: true,
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.status == "1") {
          ...
          setTimeout(() => $(".cart-" + ProductId).remove(), 500);
          $(".shopping_cart a span").text(
            `${data.sumQuantity} sản phẩm - ${data.sumMoney}`
          );
          $("#lst-cart .prices,.cart_amount.sum_money").text(data.sumMoney);
          $('.cart_amount.sum_temp_money').text(data.sumTempMoney);
          $('.cart_amount.discount').text(data.sumDiscount);
          console.log(data.sumDiscount);
  ...
  $(".txtquantity-lstcart").change(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.status == "1") {
          if (Quantity <= 0) $(".cart-" + ProductId).hide(500);
          ...
          $("#lst-cart .prices,.cart_amount.sum_money").text(data.sumMoney);
          $(".cart_amount.sum_temp_money").text(data.sumTempMoney);
          $(".cart-" + ProductId + " .cart_info .quantity").text(
            "Số lượng:" + Quantity
          );
          $(".cart_amount.sum_quantity").text(data.sumQuantity);
          $('.cart_amount.discount').text(data.sumDiscount);
  ...
  function search() {
    ...
    else location.href = "/tim-kiem?tukhoa=" + keyword;
  }
  ...
  $("#txtkeyword").keyup(function () {
    let val = $(this).val();
    if (val == null || val == "") $(".search-list").css("display", "none");
    else {
      suggestSearch(val);
  ...
  $("#rate li").each(function (index, element) {
    $(element)
      ...
      .mouseenter(function (e) {
        ...
        for (var i = 0; i < 5; i++) {
          if (i <= index) $($("#rate li")[i]).find("a").addClass("ratting");
          else if (!$($("#rate li")[i]).find("a").hasClass("active"))
  ...
  $("#rate li").each(function (index, element) {
    $(element)
      ...
      .mousedown(function (e) {
        e.preventDefault();
        for (var i = 0; i < 5; i++) {
          if (i <= index) $($("#rate li")[i]).find("a").addClass("active");
          else $($("#rate li")[i]).find("a").removeClass("active");
        }
        return false;
      });
  });
  ...
  $("#btnrate").click(function () {
    ...
    $("#rate li").each(function (index, elemet) {
      if ($(this).find("a").hasClass("active")) sum++;
    });
      ...
      success: function (data) {
        if (data.status == 1) {
          ...
          $("#countrate li").each(function (index, element) {
            if (index < data.avgStar) $(element).find("a").addClass("ratting");
            else $(element).find("a").removeClass("ratting");
          });
          $("#rate li").each(function (index, element) {
            if (index != 0) $(element).find("a").removeClass("ratting active");
          });

          var img = "";
          if (data.issocial == 0) img = `/${data.image}`;
          else img = data.image;
          var li = "";
          for (var i = 0; i < 5; i++)
            if (i < sum)
              li += `<li><a class="ratting"><i class="fa fa-star"></i></a></li>`;
            else li += `<li><a><i class="fa fa-star"></i></a></li>`;
  ...
  $("#btnsub").click(function () {
    ...
    else if (!validateEmail(sub.email))
      iziToast.warning({
        timeout: 1500,
        title: "Cảnh báo",
  ...
  $("#btn-wheel-acc").click(function () {
    location.href = "/vong-xoay-tai-khoan";
  });
  ...
  function updateAccount() {
    ...
    else if (!validateEmail(acc.email))
      iziToast.warning({
        timeout: 1500,
        title: "Cảnh báo",
    ...
    else if (!validatePhone(acc.phone))
      iziToast.warning({
        timeout: 1500,
        title: "Cảnh báo",
     ...
     $.ajax({
      ...
      enable('#btn-discount');
        ...
        if(data.status)
        {
          ...
          $('.cart_amount.discount').text(data.sumDiscount);
          $('.cart_amount.sum_money').text(data.sumMoney);
        }
        else
        iziToast.error({
          timeout: 1500,
