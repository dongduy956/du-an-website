...
$(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.status == 1) {
          ...
          $(
            "#confirm_order_detail_" + id + ",#confirm_order_" + id
          ).removeClass("text-danger");
          $("#confirm_order_detail_" + id + ",#confirm_order_" + id).addClass(
            "text-success"
          );
          $("#confirm_order_detail_" + id + ",#confirm_order_" + id).text(
            "Đã duyệt"
          );
        } else
          iziToast.error({
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.status == 1) {
          ...
          $(
            "#transfer_order_detail_" + id + ",#transfer_order_" + id
          ).removeClass("text-danger");
          $("#transfer_order_detail_" + id + ",#transfer_order_" + id).addClass(
            "text-success"
          );
          $("#transfer_order_detail_" + id + ",#transfer_order_" + id).text(
            "Đã chuyển tiền"
          );
        } else
          iziToast.error({
  ...
  function loadReceipt() {
    lstreceiptdetail =
      window.sessionStorage.getItem("receipt") == null
        ? []
        : JSON.parse(window.sessionStorage.getItem("receipt"));
    var html = "";
    $.each(lstreceiptdetail, function (index, item) {
      html += `
             <tr id='${item.idproduct}'>
                                ...
                                <td><input value='${item.quantity}' data-id="${
        item.idproduct
      }" type='number' class ='quantity-input form-control' min='0' max='100'/></td>
                                <td id='subtotal_${
                                  item.idproduct
        ...
        item.nameproduct
      }" class ="mt-1 mb-1 delete_receipt-detail d-sm-inline-block btn btn-sm btn-primary shadow-sm">
  ...
  function add_receipt_detail() {
    ...
    if (index != -1) {
      ...
      lstreceiptdetail[index].subtotal =
        lstreceiptdetail[index].quantity * price;
    } else {
    ...
    $.each(lstreceiptdetail, function (index, item) {
      html += `
             <tr id='${item.idproduct}'>
                                ...
                                <td><input value='${
                                  item.quantity
                                }' type='number' class ='form-control quantity-input' data-id='${
        item.idproduct
      }' min='0' max='100'/></td>
                                <td id='subtotal_${
                                  item.idproduct
  ...
  //Tăng/giảm số lượng/xoá 1 chi tiết phiếu nhập
  $("#table-detail").change(function (e) {
    if (e.target.closest(".quantity-input")) {
      var id = $(e.target).data("id");
      console.log(id);
      var quantity = Number($(e.target).val());
      var index = lstreceiptdetail.findIndex(function (item) {
      ...
      if (quantity <= 0) {
        ...
        lstreceiptdetail[index].quantity = quantity;
        lstreceiptdetail[index].subtotal =
          quantity * Number(lstreceiptdetail[index].price);
        $("#subtotal_" + id).text(
          number_format(quantity * Number(lstreceiptdetail[index].price))
        );
      }
      window.sessionStorage.setItem(
        "receipt",
        JSON.stringify(lstreceiptdetail)
      );
    } else if (e.target.closest(".delete_receipt-detail")) {
      e.preventDefault();
  ...
  //Hàm xoá 1 phiếu giảm giá
  function delete_promotion(id) {
    $.ajax({
      url: "/Promotion_admin/delete_promotion/" + id,
      data: JSON.stringify(id),
      contentType: "application/json; charset=utf-8",
      dataType: "json",
      type: "POST",
  ...
  //Event gọi hàm xoá 1 phiếu giảm giá
  $(".delete_promotion")
    .off("click")
    .click(function (e) {
      e.preventDefault();
      var id = Number($(this).data("id"));
      swal({
        title: "Bạn chắc chắn xoá phiếu giảm giá này?",
        icon: "warning",
        buttons: true,
        dangerMode: true,
      }).then((willDelete) => {
  ...
  function loadImages(id) {
    $.ajax({
      ...
      success: function (res) {
        var html = "";
        var images = window.sessionStorage.setItem(
          "images",
          JSON.stringify(res.data)
        );
        $.each(res.data, function (index, item) {
          html += `<div><img width='100' src='/${item}'> <a href='' class='btn-delete-image'><i class='fa fa-times'></i></a></div>`;
        });
  ...
  //xoá image trong danh sách image
  $("#lstimage").click((e) => {
    e.preventDefault();

    if (e.target.closest(".btn-delete-image")) {
      var images =
        window.sessionStorage.getItem("images") == null ||
        window.sessionStorage.getItem("images") == '""'
          ? []
          : JSON.parse(window.sessionStorage.getItem("images"));
      $(e.target.parentElement).parent().remove();
      var src = $(e.target.parentElement).parent().find("img").attr("src");
  ...
  $("#btnselectimage").click(function () {
    ...
    finder.selectActionFunction = function (url) {
      ...
      if (check) {
        ...
        window.sessionStorage.setItem("images", JSON.stringify(images));
        $("#lstimage").append(
          `<div><img width='100' src='${url}'> <a href='' class='btn-delete-image'><i class='fa fa-times'></i></a></div>`
        );
      }
    //   $(".btn-delete-image")
    //     .off("click")
